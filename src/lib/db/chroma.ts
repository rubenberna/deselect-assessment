...
import {createEmbeddingsFromChunks, embedder} from "@/src/lib/ai/embeddings";
...
export async function getPDFCollection() {
  ...
  return await client.getOrCreateCollection({
    name: 'pdf_data',
    embeddingFunction: embedder, // Used if no embeddings are provided
  })
}
...
export async function insertPDFChunks(chunks: Document[]) {
  ...
  const {embeddings} = await createEmbeddingsFromChunks(chunks)
